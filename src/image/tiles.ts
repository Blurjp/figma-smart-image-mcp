...
import { ImageEncoder, EncodedImage, EncodeOptions } from "./encode.js";

export interface Tile {
  path: string;
  x: number;
  y: number;
  w: number; // Width in original image coordinates
  h: number; // Height in original image coordinates
  bytes: number;
...
export class ImageTiler {
  constructor(private readonly encoder: ImageEncoder) {}
  ...
  async generateTiles(
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const encoded = await this.encoder.encodeToFit(
          inputPath,
          tilePath,
          {
            maxBytes,
...
ImageEncoder.prototype.encodeToFit = async function(
  this: ImageEncoder,
  inputPath: string,
  outputPath: string,
  options: EncodeOptions,
