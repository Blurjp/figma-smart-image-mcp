...
import { ImageEncoder, EncodeOptions } from "./encode.js";
...
export class ImageCropper {
  ...
  async generateCrops(
    ...
    const cropRegions = this.generateCropRegions(
      imageWidth,
      imageHeight,
      minCropSize
    );
    ...
    for (const region of cropRegions) {
      const cropPath = join(
        outputDir,
        `crop_${cropIndex}_${this.sanitizeName(region.name)}.${preferFormat === "webp" ? "webp" : "jpg"}`
      );
